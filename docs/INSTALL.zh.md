...
快照包含**能力配置**：推理强度、兼容开关，以及哪些模型启用会话 Header，可以安全地在不同机器之间迁移。快照不包含**部署接线**：provider 的 `baseURL`、`apiKeyEnv`、`headers` 以及 `opencodeSession.format.script` 在导入时会被扣留，避免他人提供的文件重定向请求、挂接其凭据名称、注入原始 Header，或指定供 Host 导入并执行的本地模块。
