...
在 `0.1.7` 之前的版本上，本插件把 `subagentEffort` 写进 DSH 设置文档的 `llm-pi-ai` 分区。DSH `0.1.7` 会把该文档重命名为 `settings.yaml.imported` 并只导入一次；对本插件的分区而言，这次导入会被拒绝并只留下警告，因此旧值只保留在这个重命名后的文件里。它就位于 DSH 主目录下：`$DSH_HOME/settings.yaml.imported`（默认是 `~/.dsh/settings.yaml.imported`）。`0.1.7` 及以后，活的设置不再存放在该文件，而是存放在当前 profile 的 `cordis.patch.yml`。

插件会在启动时扫描 `settings.yaml.imported`、尚未重命名的 `settings.yaml` 以及活的 `llm-pi-ai` 用户层。当发现属于本插件、而你的设置分区尚未声明的值时，会在设置页上方弹出询问。点击「迁移」即可把它们写入本插件的设置分区；迁移只补齐你尚未设置的值、绝不覆盖，并会把当时的设置存进「配置备份与方案」的回滚槽（`autoBackup`，来源标记为 `migration`），与迁移值在同一批写入中落盘。「不再询问」会屏蔽这一次的具体内容，仅当旧数据发生变化才会再次提示——之后文档才变化的机器仍会收到提示。

想按需检查旧数据时，打开设置页，使用「配置备份与方案」卡片中的「重新扫描旧数据」；这就是「不再询问」之后的手动入口。当文档中已经没有本插件尚未声明的项时，它会提示「没有待迁移的旧数据」。
