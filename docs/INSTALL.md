...
The Host derives the Header value from the current DSH conversation's `sessionId` for each matching `llm/stream` request. You do not enter or store a fixed value. Models that share the route, including GPT or other non-OpenCode models, do not inherit the setting, and the setting does not change the route's `api` protocol.
