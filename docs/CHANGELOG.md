...
- 修复 Composer 思考面板在窄屏上定位出界：面板此前锚定触发器右边缘向左展开 336px，手机上模型芯片贴着屏幕左边缘，整个面板被甩出屏幕、基本点不到；单纯把锚点改成左边缘又会在芯片离左边较远时从右侧溢出一截——CSS 的 `min(336px, 100vw - 32px)` 只限宽度、限不了起始位置。现在默认锚点改为左边缘，面板打开时量一次触发器实际位置，把面板 clamp 到 `[16px, 视口宽 - 16px]` 区间内，窗口 resize 与页面滚动时重算。桌面端算出的偏移是 0，行为与之前一致。
- Fix the composer thinking panel overflowing the viewport on narrow screens: it was anchored to the trigger's right edge and opened 336px leftward, so on phones — where the model chip sits flush against the left edge of the screen — the whole panel landed off-screen and was essentially untappable; anchoring it to the left edge alone would instead let it overflow the right side whenever the chip sits more than a panel-width from the edge, because the CSS `min(336px, 100vw - 32px)` bounds the width but not the starting position. The default anchor is now the left edge, and on open the panel measures the trigger's actual position and clamps itself into `[16px, viewport width - 16px]`, recomputed on window resize and page scroll. On desktop the computed offset is 0, so behavior is unchanged.
