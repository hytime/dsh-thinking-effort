...
- 公開前の互換マトリクスに `dsh-v0.1.7-alpha.1`（entry-config 設定モデル）を 5 つ目の公式 capability representative として追加しました。0.1.7 のルートでは、プラグインが実際にロードされること、設定セクションが公開されること、およびホストの 2 つの不変条件（`llm-pi-ai` の `providers` が volatile のままであること、パス書き込みがユーザーの生レイヤーから導出されること）を検証します。
...
- DSH 0.1.7 の entry-config 設定モデルに対応しました。プラグインがルート `.volatile()` の `Config` を公開するようになり、このリリースで設定フォームが表示されるようになります（以前は 0.1.7 で `register is missing` により読み込みに失敗していました）。`0.1.7` 以降の設定セクションは Loader エントリ ID の `thinking-effort`、`0.1.0-rc.7` から `0.1.6` までは登録済み namespace の `dsh-thinking-effort` のままで、Client は実行中の Host が公開している方を解決します。`subagentEffort` は本プラグイン自身の設定セクションへ移り（0.1.7 の `llm-pi-ai` schema は `providers` しか宣言しないため、旧位置には書き込めなくなりました）、Host は旧 `llm-pi-ai` の場所もフォールバックとして読みます。設定は `~/.dsh/settings.yaml` ではなく現在の profile の `cordis.patch.yml` に保存されます（`0.1.7` は旧ファイルを自動で移行またはリネームします）。
