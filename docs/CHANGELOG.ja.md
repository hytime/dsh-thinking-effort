...
- 推理等級のスライダーが 1 回のドラッグで 1 段階しか動かせない問題を修正しました。公式モデルディレクトリの `select()` は `async` 関数で、最初の `await` より前に同期的に状態を `selecting` にします。パネルはこれを「処理中」と見なしてネイティブの range を無効化し、以後ブラウザはそのドラッグの残りの入力を報告しませんでした。サービスは連続選択のために設計されています（`select()` ごとに generation を取得し最新の応答のみが反映され、成功時・失敗時の両方で `pending` がクリアされます）ので、連続的なコントロールを無効化していたのは UI 層のセマンティクスの取り違えです。スライダーが無効化されるのは、パネルがロックされた場合か `select` が存在しない場合のみで、離散的なコントロール（モデル行とその選択肢）は従来どおりの処理中状態を維持します。スライダーは制御された入力なので、位置はディレクトリストアの `pending`（`select()` が同期的に書き込む、ホストが受け付けたが未確認の選択）を読み、早いディレクトリでは `current` にフォールバックします。前半だけの修正ではドラッグ中にスライダーがホストの古い値へ戻ってしまいます。パネルは `pending` を読むだけで書き込まず、ホストストアのビューであり続けます。
