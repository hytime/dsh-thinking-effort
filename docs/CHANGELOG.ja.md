...
- 旧データの自動検出と移行を追加しました。DSH 0.1.7 は `settings.yaml` の名前を変更して一度だけ取り込みますが、以前のバージョンのプラグインはそのリリースで読み込みに失敗していたため、それまで設定した思考強度とモデルスイッチは `settings.yaml.imported` にだけ残っています。プラグインは起動時にこのファイル（およびまだ名前が変更されていない `settings.yaml` と、稼働中の `llm-pi-ai` ユーザーレイヤー）をスキャンし、移行できる項目を見つけると設定ページの上で確認を表示し、あなたが承認してからプラグイン自身の設定セクションへ書き込みます。移行はまだ設定していない値だけを補い、上書きは決してしません。書き込む前にその時点の設定を「設定のバックアップとプロファイル」のロールバック枠（`autoBackup`、`sourceProfile: migration`）へ保存し、スナップショットと移行値は同じ書き込みバッチでディスクに書かれるため、「スナップショットだけがあって移行がない」中間状態は生じません。「今後表示しない」を選んだ後は、旧データが変化したときだけ再び確認します。
