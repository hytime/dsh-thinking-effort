...
- 設定ページに「セッション値ジェネレーター」カードを追加しました。UI から `opencodeSession.format` のすべてのフィールド（生成モード、タイムスタンプの取得元、テンプレート / 式 / スクリプトのパス、検証用正規表現、検証失敗時の動作）を読み書きできるため、設定ドキュメントを手で編集する必要はありません。カードは現在のモードで使うフィールドだけを表示し、書き込み前に検証します。不正な正規表現、空のテンプレート / 式 / スクリプト、相対スクリプトパス、解析できない式は、いずれも理由付きで拒否されます。これらの値はホスト側で黙ってフォールバックするため（正規表現は「検証なし」、それ以外は派生値）、UI で見えないままだと誤解を招きます。式の検証はホストと同じ解析コードを共有します。書き込みはフィールド単位で行われるため、同じ namespace の他の設定を上書きしません。
