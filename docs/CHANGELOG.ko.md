...
- 「설정 백업 및 프로필」을 추가했습니다. `llm-pi-ai`와 `dsh-thinking-effort` 두 namespace의 사용자 레이어를 JSON 파일로 내보내 다른 머신이나 재설치 후 가져와 복원할 수 있습니다. 설정 페이지에서 이름 있는 프로필을 여러 개 저장하고 전환할 수도 있습니다. 가져올 때는 반드시 「병합」(기본, 파일에 없는 provider 유지) 또는 「교체」(파일 내용 기준)를 선택하며 추가 / 덮어쓰기 / 삭제 건수 미리보기를 표시합니다. 적용 전에는 롤백용 가져오기 전 스냅샷을 자동으로 저장합니다. 기존 Settings 채널을 재사용하므로 최신 Remote Settings와 이전 `connection.api.settings`에서 모두 동작하며 새로운 의존성이 없습니다.
