...
- 추론 등급 슬라이더가 한 번의 드래그로 한 단계만 이동하던 문제를 수정했습니다. 공식 모델 디렉터리의 `select()`는 `async` 함수여서 첫 `await` 이전에 동기적으로 상태를 `selecting`으로 만듭니다. 패널은 이를 「처리 중」으로 판단해 네이티브 range를 비활성화했고, 그 이후 브라우저는 같은 드래그의 나머지 입력을 보고하지 않았습니다. 서비스는 연속 선택을 위해 설계되어 있습니다(각 `select()`가 generation을 얻고 최신 응답만 적용되며, 성공과 실패 경로 모두 `pending`을 정리합니다). 따라서 연속控件을 비활성화한 것은 UI 계층의 의미 불일치였습니다. 이제 슬라이더는 패널이 잠겼거나 `select`가 아예 없을 때만 비활성화되고, 이산적인控件(모델 행과 그 선택지)은 기존 처리 중 상태를 유지합니다. 슬라이더는 제어 입력이라 위치는 디렉터리 저장소의 `pending`을 읽습니다. 이는 `select()`가 동기적으로 기록하는, 호스트가 받아들였지만 아직 확인하지 않은 선택이며, 해당 필드가 없는 이전 디렉터리에서는 `current`로 대체됩니다. 앞 절반만 고치면 드래그 도중 슬라이더가 호스트의 낡은 값으로 되돌아가므로 둘 다 필요합니다. 패널은 `pending`을 읽기만 하고 쓰지 않으며, 호스트 저장소의 뷰로 남습니다.
