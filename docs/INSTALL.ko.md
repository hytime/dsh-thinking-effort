...
`0.1.7` 이전 릴리스에서 이 플러그인은 `subagentEffort`를 DSH 설정 문서의 `llm-pi-ai` 섹션에 기록했습니다. DSH `0.1.7`은 그 문서를 `settings.yaml.imported`로 이름을 바꾸고 한 번만 가져오는데, 이 플러그인의 섹션에 대해서는 그 가져오기가 경고만 남기고 거부되므로 이전 값은 이름이 바뀐 파일에만 남습니다. 위치는 DSH 홈 바로 아래, `$DSH_HOME/settings.yaml.imported`(기본값 `~/.dsh/settings.yaml.imported`)입니다. `0.1.7` 이상에서 실행 중인 설정은 이 파일이 아니라 현재 profile의 `cordis.patch.yml`에 저장됩니다.

플러그인은 시작할 때 `settings.yaml.imported`, 아직 이름이 바뀌지 않은 `settings.yaml`, 그리고 실행 중인 `llm-pi-ai` 사용자 레이어를 검사합니다. 이 플러그인이 소유한 값 중 사용자의 설정 섹션이 아직 선언하지 않은 것을 찾으면 설정 페이지 위에 확인 창을 띄웁니다. **마이그레이션**을 누르면 이 플러그인의 설정 섹션에 기록됩니다. 마이그레이션은 아직 설정하지 않은 값만 채우고 덮어쓰지 않으며, 그 시점의 설정을 **설정 백업 및 프로필**의 롤백 슬롯(`autoBackup`, `sourceProfile: migration`)에 마이그레이션 값과 같은 쓰기 배치로 저장합니다. **다시 묻지 않기**는 그 시점의 내용을 기록해 확인을 억제하므로, 이후 이전 데이터가 바뀔 때만 다시 묻습니다(나중에 문서가 바뀐 머신도 다시 묻습니다).

필요할 때 이전 데이터를 확인하려면 설정 페이지를 열고 **설정 백업 및 프로필** 카드의 **이전 데이터 다시 검사**를 사용하세요. 이것이 **다시 묻지 않기**를 선택한 뒤의 수동 진입점입니다. 이 플러그인이 아직 선언하지 않은 항목이 문서에 남아 있지 않으면 "마이그레이션할 이전 데이터가 없습니다"라고 표시됩니다.
...
```bash
dsh plugin --profile <profile> add @hytime/dsh-thinking-effort@0.3.4
```
