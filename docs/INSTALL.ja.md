...
`0.1.7` より前のリリースでは、本プラグインは `subagentEffort` を DSH 設定ドキュメントの `llm-pi-ai` セクションへ書き込んでいました。DSH `0.1.7` はそのドキュメントを `settings.yaml.imported` にリネームして一度だけ取り込みますが、本プラグインのセクションではこの取り込みが警告だけを残して拒否されるため、古い値はリネーム後のファイルにだけ残ります。場所は DSH ホームの直下、`$DSH_HOME/settings.yaml.imported`（既定では `~/.dsh/settings.yaml.imported`）です。`0.1.7` 以降、稼働中の設定はこのファイルではなく現在の profile の `cordis.patch.yml` に保存されます。

プラグインは起動時に `settings.yaml.imported`、まだリネームされていない `settings.yaml`、そして稼働中の `llm-pi-ai` ユーザーレイヤーをスキャンします。本プラグインが所有する値で、あなたの設定セクションがまだ宣言していないものを見つけると、設定ページの上に確認を表示します。**移行** を押すと本プラグインの設定セクションへ書き込まれます。移行はまだ設定していない値だけを補い、上書きはしません。また、その時点の設定を **設定のバックアップとプロファイル** のロールバック枠（`autoBackup`、`sourceProfile: migration`）へ、移行値と同じ書き込みバッチで保存します。**今後表示しない** はその時点の内容を記録して確認を抑止するため、旧データが変化したときだけ再び表示されます（後からドキュメントが変わったマシンでも再び確認されます）。

必要なときに旧データを調べるには、設定ページを開き、**設定のバックアップとプロファイル** カードの **旧データを再スキャン** を使ってください。これが **今後表示しない** を選んだ後の手動入口です。本プラグインがまだ宣言していない項目がドキュメントに残っていなければ、「移行待ちの旧データはありません」と表示されます。
...
このリリースではバージョンが `0.3.2` である必要があります。
