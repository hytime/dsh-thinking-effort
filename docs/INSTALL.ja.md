...
Host は一致する `llm/stream` リクエストごとに、現在の DSH 会話の `sessionId` から Header 値を動的に生成します。固定値を入力または保存する必要はありません。同じルートの GPT など OpenCode 以外のモデルには継承されず、ルートの `api` プロトコルも変更しません。
...
公開前に workflow は rc7 → rc2 → alpha2 の順で、3 つの一時的な公式 DSH capability representative checkout を作成します。公式 `dsh plugin` コマンドで現在の tarball をインストールしてから、実際の互換性テストを実行します。
