 ...
 * 数据经 DSH Settings Remote 读写，无需自定义 remote。
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 将旧版 RPC 包装响应和新版 Remote 结果统一为 ClientResult。 */
    ...
    function settingsBridge(context, connection, injected) {
      ...
      if (modernSettings && typeof modernSettings.describe === 'function'
        ...
        return {
          describe: () => modernSettings.describe().then(directResult),
          mutate: (ns, ops, expectedRevision) => modernSettings
            .mutate(ns, ops, expectedRevision).then(directResult),
        };
      }
      const legacySettings = connection && connection.api && connection.api.settings;
      if (legacySettings && typeof legacySettings.describe === 'function'
        && typeof legacySettings.mutate === 'function') {
        return {
          describe: () => legacySettings.describe({}).then(directResult),
          mutate: (ns, ops, expectedRevision) => legacySettings
            .mutate({ ns, ops, expectedRevision }).then(directResult),
        };
      }
      return undefined;
    ...
    function SectionEditor(props) {
      ...
      const load = () => {
        ...
        settings.describe().then((response) => {
          if (!response.ok) {
            setState(s => ({ ...s, loading: false, busy: false, error: response.error.message }));
            return;
          }
      ...
      const runOps = (ops, successMessage, onSuccess) => {
        ...
        settings.mutate(NS, ops, state.revision).then((response) => {
          if (!response.ok) {
            setState(s => ({ ...s, busy: false, error: t('writeError', { message: response.error.message }) }));
            return;
          }
          const nextNamespace = response.value;
      ...
      const applySubagentEffort = () => {
        ...
        setState(s => ({ ...s, busy: true, error: null, notice: null }));
        settings.mutate(NS, ops, state.subagent ? state.subagent.revision : 0).then((response) => {
          if (!response.ok) {
            setState(s => ({ ...s, busy: false, error: t('writeError', { message: response.error.message }) }));
            return;
          }
      ...
      apply(ctx) {
        ...
        const mount = (scope) => {
          ...
          ctx.effect(() => {
            ...
            const disposeDictionaries = locale.register(LOCALE_NS, { zh, en, ja, ko });
            const hasLanguage = (id) => {
              const snapshot = typeof locale.getSnapshot === 'function' ? locale.getSnapshot() : undefined;
              return Array.isArray(snapshot && snapshot.locales)
                && snapshot.locales.some(entry => entry && entry.id === id);
            };
            try {
              if (typeof locale.addLanguage === 'function' && !hasLanguage('ja')) {
                disposeLanguages.push(locale.addLanguage({ id: 'ja', label: '日本語', fallback: 'en' }));
              }
              if (typeof locale.addLanguage === 'function' && !hasLanguage('ko')) {
                disposeLanguages.push(locale.addLanguage({ id: 'ko', label: '한국어', fallback: 'en' }));
              }
            } catch (error) {
              for (const dispose of disposeLanguages.reverse()) dispose();
              disposeDictionaries();
              throw error;
            }
            return () => {
