...
export function ModelRow({ item, open, draft, contextDraft, inputDraft, dirty, busy, palette, t, onToggle, onLevelChange, onContextChange, onOneMillionChange, onInputChange, onSave, onRestoreReasoning, onRestoreCapability, compatView, onCompatChange, onSaveCompat, compatDirty, compatExpanded, onToggleCompatExpanded, openCodeSession, openCodeSessionDirty, openCodeSessionAvailable, onOpenCodeSessionChange, onSaveOpenCodeSession }: ModelRowProps): React.ReactElement {
  ...
  const hasOpenCodeSessionDirty = openCodeSessionDirty === true
  return <div style={{ border: `1px solid ${open ? palette.accent : dirty || hasCompatDirty || hasOpenCodeSessionDirty ? palette.accentBorder : palette.border}`, borderRadius: '8px', marginBottom: '4px', backgroundColor: open ? palette.raised : palette.group, boxShadow: palette.shadow, overflow: 'hidden', transition: 'background-color 160ms ease, border-color 160ms ease' }}>
