...
function settingsBridge(connection, remoteSettings) {
	...
	const compatibility = resolveCompatibility({ capabilities: clientCapabilities({
		remoteSettings,
		legacySettings
	}) });
	...
	if (compatibility.profile === "legacy" && legacySettings !== void 0) {
...
function apply(context) {
	...
	const mount = (settings) => {
		...
		context.effect(() => {
			...
			try {
				if (capabilities.externalLanguages && !hasLanguage(locale, "ja")) languageDisposers.push(locale.addLanguage({
					id: "ja",
					label: translate("languageJapanese"),
					fallback: "en"
				}));
				if (capabilities.externalLanguages && !hasLanguage(locale, "ko")) languageDisposers.push(locale.addLanguage({
					id: "ko",
	...
	mount(settingsBridge(connection, void 0));
