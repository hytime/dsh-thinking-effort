...
DSH 0.1.7 会把 `settings.yaml` 重命名并只导入一次，而更早版本的插件在 0.1.7 上加载失败，因此你此前设置的思考档位或模型开关可能只留在 `settings.yaml.imported` 里。插件会在启动时扫描该文档、尚未重命名的 `settings.yaml` 以及活的 `llm-pi-ai` 用户层；当发现属于本插件、而你的设置分区尚未声明的值时，会在设置页上方弹出询问：「迁移」把它们写入本插件的设置分区，「稍后」在本次页面加载内不再追问，「不再询问」则记录下这一次的具体内容，仅当旧数据之后发生变化才会再次提示。在你点击「迁移」之前不会写入任何内容，且迁移只补齐你尚未设置的值，绝不覆盖。写入前，插件会把当时的设置存进「配置备份与方案」的回滚槽（`autoBackup`，来源标记为 `migration`），并与迁移值在同一批写入中落盘。想按需检查旧数据时，使用「配置备份与方案」卡片中的「重新扫描旧数据」。
